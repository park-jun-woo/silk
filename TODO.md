...
코드북 산출(타입 정의, 비트 할당, 인코딩, 검증)은 별도 프로젝트 **geul-sidx** 범위.
SILK은 geul-sidx가 생성한 data/ 산출물만 소비한다.
...
**geul-sidx** 프로젝트가 생성한 산출물을 data/에 배치. 모든 후속 작업의 전제조건.

SILK은 코드북을 만들지 않는다. geul-sidx의 결과물을 가져다 쓴다.

### geul-sidx → data/ 연동
- [ ] geul-sidx에서 산출물 복사 또는 심볼릭 링크
...
silk/ 패키지. geul-sidx 산출물(data/)을 로드하여 검색 수행.
...
1. **geul-sidx → data/**: 코드북 산출물 배치 (geul-sidx 프로젝트 선행)
